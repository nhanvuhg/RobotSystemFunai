...
public:
      ...
      max_boxes), _filter_by_score(filter_by_score), _vstream_info(tensor->vstream_info())
  {
    ...
    // }// Một số phiên bản HailoRT mới trả về order khác cho NMS (vd. 22 – BY_CLASS)
    constexpr int kLegacyNms = HAILO_FORMAT_ORDER_HAILO_NMS;
    constexpr int kByClassNmsGuess = 22; // quan sát từ log của bạn

    const auto order = static_cast<int>(_vstream_info.format.order);
    if (!(
          order == kLegacyNms
      || order == kByClassNmsGuess
      /*|| order == HAILO_FORMAT_ORDER_HAILO_NMS_PER_CLASS */ // nếu SDK bạn có hằng số này
      )) {
        throw std::invalid_argument(
            "Output tensor " + _nms_output_tensor->name() + " is not an NMS type (order=" + std::to_string(order) + ")");
    }
  }
