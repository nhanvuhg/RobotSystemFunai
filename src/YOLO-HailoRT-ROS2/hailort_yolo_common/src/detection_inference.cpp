...
// -----------------------------
// VDevice dùng chung (singleton)
// -----------------------------
namespace {
// VDevice dùng chung (singleton) – an toàn luồng
    std::shared_ptr<hailort::VDevice> get_shared_vdevice()
    {
        static std::weak_ptr<hailort::VDevice> weak_vdev;
        static std::mutex mtx;

        std::lock_guard<std::mutex> lock(mtx);

        // Đã có shared VDevice?
        auto sp = weak_vdev.lock();
        if (sp) {
            return sp;
        }
        // Tạo mới
        ...
        // Chuyển unique_ptr -> shared_ptr đúng chuẩn C++
        std::unique_ptr<hailort::VDevice> up = std::move(vdev_exp.value());
        sp = std::shared_ptr<hailort::VDevice>(std::move(up)); // ctor nhận unique_ptr&&
    ...
    std::shared_ptr<VStreams> &vstreams)
{
    ...
    // Dùng VDevice dùng chung thay vì tạo mới mỗi node
    try 
    {
    ...
    {
        std::cerr << "[HailoRT] " << e.what() << std::endl;
