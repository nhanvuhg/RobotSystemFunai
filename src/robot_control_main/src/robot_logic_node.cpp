...
public:
    RobotLogicNode() : Node("robot_logic_nova5"), detected_(false), has_button_(false), current_active_camera_(-1)
    {
        ...
        iou_thresh_ = this->declare_parameter<double>("iou_thresh", 0.05); // Lowered threshold
...
private:
    ...
    void step_7_check_tray_empty_position()
    {
        ...
        {
            RCLCPP_WARN(this->get_logger(), "[STEP 7] ⚠️ Camera 1 Active but NO empty slot detected (slot_id=%d). Retrying...", slot_id);
            current_step_ = Step::step_7_check_tray_empty_position;
        } 
    }
    ...
    void init_rows_and_rois()
    {
        std::vector<std::vector<std::pair<int, int>>> all_corners = {
            // row_1
            {{735, 450}, {711, 102}, {792, 103}, {857, 450}},
            // row_2
            {{600, 447}, {615, 104}, {706, 103}, {728, 451}},
            // row_3
            {{474, 447}, {529, 104}, {611, 105}, {598, 448}},
            // row_4
            {{350, 452}, {440, 107}, {520, 108}, {470, 449}},
            // row_5
            {{207, 451}, {341, 107}, {435, 107}, {343, 452}},
            };
        ...
        for (const auto &det : msg->detections)
        {
            ...
            for (const auto &r : det.results)
            {
                ...
                for (size_t i = 0; i < 6; ++i)
                {
                    ...
                    {
                        // Row 6 (Scale) - Global detection or specific ROI
                        // Here we use class "2" and "3" for scale status
                        if (class_id == "2")
                            row6_cnt2_++;
                        else if (class_id == "3")
                            row6_cnt3_++;
    ...
    void buildCanonicalSlots()
    {
        ...
        canonical_slots_.push_back(SlotDef{{{210, 530}, {250, 415}, {600, 415}, {600, 530}}, 'H'});
        // row2
        canonical_slots_.push_back(SlotDef{{{250, 415}, {280, 322}, {600, 322}, {600, 415}}, 'H'});
        // row3
        canonical_slots_.push_back(SlotDef{{{280, 322}, {306, 225}, {600, 225}, {600, 322}}, 'H'});
        // row4
        canonical_slots_.push_back(SlotDef{{{334, 225}, {357, 133}, {626, 133}, {630, 225}}, 'H'});
        // row5 (middle vertical)
        canonical_slots_.push_back(SlotDef{{{597, 519}, {605, 233}, {705, 233}, {722, 519}}, 'V'});
        // row6 (User Defined)
        canonical_slots_.push_back(SlotDef{{{715, 530}, {715, 415}, {1080, 415}, {1105, 530}}, 'H'});
        // row7 (User Defined)
        canonical_slots_.push_back(SlotDef{{{715, 415}, {715, 315}, {1060, 315}, {1090, 415}}, 'H'});
        // row8 (User Defined)
        canonical_slots_.push_back(SlotDef{{{700, 315}, {700, 220}, {1030, 220}, {1040, 315}}, 'H'});
        // row9 (User Defined)
        canonical_slots_.push_back(SlotDef{{{680, 215}, {680, 135}, {1010, 135}, {1010, 215}}, 'H'});
    }
    ...
    void detCallback(const Detection2DArray::SharedPtr msg)
    {
        ...
        for (int s = 0; s < (int)slots.size(); ++s)
        {
            ...
            for (int d = 0; d < (int)dets.size(); ++d)
            {
                ...
                // Lower threshold to catch offset boxes
                if (!inside && iou < 0.05) 
                    continue;
