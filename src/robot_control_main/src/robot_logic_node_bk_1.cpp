...
enum class SlotStableState : int { EMPTY=0, OCC_OK=1, MIS=2 };
...
private:
    ...
    void init_rows_and_rois()
    {
        std::vector<std::vector<std::pair<int, int>>> all_corners = {
            // row_1
            {{108, 391}, {170, 270}, {199, 273}, {143, 394}},
            // row_2
            {{160, 296}, {164, 223}, {611, 218}, {630, 283}}
            // row_3
            {{279, 397}, {300, 261}, {334, 263}, {330, 396}},
            // row_4
            {{231, 397}, {267, 262}, {296, 265}, {280, 396}},
            // row_5
            {{187, 396}, {231, 266}, {262, 267}, {230, 397}},
            // row 6
            {{146, 393}, {200, 269}, {229, 271}, {183, 398}}};
    ...
    void buildCanonicalSlots()
    {
        ...
        // point1
        canonical_slots_.push_back(SlotDef{{{72, 459}, {72, 395}, {276, 395}, {276, 460}}, 'H'});
        // point2
        canonical_slots_.push_back(SlotDef{{{84, 390}, {84, 327}, {278, 327}, {277, 277}}, 'H'});
        // point3
        canonical_slots_.push_back(SlotDef{{{95, 324}, {93, 267}, {280, 266}, {280, 325}}, 'H'});
        // point4
        canonical_slots_.push_back(SlotDef{{{118, 260}, {119, 210}, {297, 208}, {295, 260}}, 'H'});
        // point5 (middle vertical)
        canonical_slots_.push_back(SlotDef{{{282, 452}, {281, 277}, {357, 277}, {356, 450}}, 'V'});
        // point6
        canonical_slots_.push_back(SlotDef{{{361, 455}, {362, 396}, {572, 395}, {575, 455}}, 'H'});
        // point7
        canonical_slots_.push_back(SlotDef{{{362, 392}, {360, 330}, {568, 331}, {567, 393}}, 'H'});
        // point8
        canonical_slots_.push_back(SlotDef{{{356, 323}, {357, 270}, {551, 271}, {550, 324}}, 'H'});
        // point9
        canonical_slots_.push_back(SlotDef{{{333, 256}, {335, 209}, {512, 208}, {513, 257}}, 'H'});
    }
    ...
    void detCallback(const Detection2DArray::SharedPtr msg)
    {
        ...
        for (size_t i = 0; i < msg->detections.size(); ++i)
        {
            ...
            dets.push_back({b, o, i});

            if (debug_logs_)
            {
                RCLCPP_INFO(this->get_logger(),
                            "Detection: class=%d, score=%.2f, box=(%.1f,%.1f,%.1f,%.1f), orient=%c",
                            cid, d.results[0].hypothesis.score, b.x1, b.y1, b.x2, b.y2, o);
            }
        }
        ...
        for (int s = 0; s < (int)slots.size(); ++s)
        {
            ...
            else
            {
                ...
                if (dets[d].orient != slots[s].orient)
        ...
        RCLCPP_INFO(this->get_logger(), "Empty=%s | Misoriented=%s",
                    vecToStr(empty_slots).c_str(), vecToStr(mis_slots).c_str());
    }
