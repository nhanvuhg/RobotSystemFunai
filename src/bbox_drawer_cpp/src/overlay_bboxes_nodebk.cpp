...
public:
    ...
    : Node("overlay_"+ns, opts), ns_(ns)
    {
        ...
        out_w_       = declare_parameter<int>("output_width",  960);
        out_h_       = declare_parameter<int>("output_height", 540);
...
public:
    ...
    : Node("overlay_two_cams", opts)
    {
        ...
        cam0_ = std::make_shared<OverlayForOneCam>("cam0", child_opts);
        cam1_ = std::make_shared<OverlayForOneCam>("cam1", child_opts);
