...
public:
    CSICameraNode() : Node("csi_camera_node"), is_switching_(false), current_cam_id_(0), rpicam_pipe_(nullptr)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "========================================");
        // Default Mapping: Cam 0 = Port 2, Cam 1 = Port 1
        this->declare_parameter("cam0_channel", 2);
        this->declare_parameter("cam1_channel", 1);
...
private:
    ...
    bool safe_hardware_switch(int cam_id)
    {
        ...
        int cam0_ch = this->get_parameter("cam0_channel").as_int();
        int cam1_ch = this->get_parameter("cam1_channel").as_int();

        // DEBUG: Print loaded parameters
        RCLCPP_WARN(this->get_logger(), "🔍 CONFIG: Cam0_Ch=0x%02x, Cam1_Ch=0x%02x", cam0_ch, cam1_ch);

        if (cam_id == 0) {
            std::ostringstream cmd_ss;
            cmd_ss << "i2ctransfer -f -y 4 w3@0x0c 0xff 0x55 0x" << std::hex << std::setw(2) << std::setfill('0') << cam0_ch;
            cmd = cmd_ss.str(); 
            RCLCPP_INFO(this->get_logger(), "  🔧 [MAPPING] Cam 0 -> Channel 0x%02x (CMD: %s)", cam0_ch, cmd.c_str());
            stabilization_ms = 500;
        } else if (cam_id == 1) {
            std::ostringstream cmd_ss;
            cmd_ss << "i2ctransfer -f -y 4 w3@0x0c 0xff 0x55 0x" << std::hex << std::setw(2) << std::setfill('0') << cam1_ch;
            cmd = cmd_ss.str();
            RCLCPP_INFO(this->get_logger(), "  🔧 [MAPPING] Cam 1 -> Channel 0x%02x (CMD: %s)", cam1_ch, cmd.c_str());
            stabilization_ms = 500;
        } else {
        ...
        RCLCPP_INFO(this->get_logger(), "  📤 Executing switch...");
        int ret = std::system(cmd.c_str());
        if (ret == 0) RCLCPP_INFO(this->get_logger(), "  ✅ Switch Command Success");
        else RCLCPP_ERROR(this->get_logger(), "  ❌ Switch Command FAILED (Ret: %d)", ret);
    ...
    void start_stream()
    {
        ...
        // Build rpicam-vid command with optimizations (matching USB camera settings)
        std::string cmd = "rpicam-vid -t 0 --nopreview --codec yuv420 " +
                         ...
                         "--brightness 0.2 " +  // Brightness +20% (match USB camera brightness: 20)
